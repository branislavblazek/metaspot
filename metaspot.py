...
class Metaspot:
	...
	def render(self):
		...
		if not self.selected_game or not self.selected_level:
			version_text = "version: {}{}".format(self.version, " [dev]" if self.is_dev else "")
			self.canvas.create_text(c.HALF_WIDTH,100, text="://metaspot",font=c.HEADING_FONT, fill='green')
			self.canvas.create_text(10,570, text=version_text, font=c.SMALL_FONT, fill="green", anchor="nw")
			...
			if self.selected_game['value'] == c.START_HANGMAN: Hangman(canvas, self.selected_level, self.handle_unlock_next_level, self.handle_after_hangman, self.is_dev)
