	...
	def render(self, index=0):
		...
		if index == 0:
			for index_letter in range(len(TEXT)):
				tag = 'game_over_even' if index_letter % 2 == 0 else 'game_over_odd'
				y = 610 if index_letter % 2 == 0 else -10
				self.canvas.create_text(120 + 110 * index_letter, y, text=TEXT[index_letter], font=c.EXTRA_FONT, fill='green', tag=tag)
