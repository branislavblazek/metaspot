...
class Intro:
    ...
    def render(self):
        ...
        if self.active_choice == None:
            self.canvas.create_text(50, 250, text="Welcome here, {}! [x{}]".format(self.name or '[no_username]', self.score), font=c.REGULAR_FONT, fill='green', anchor='nw', tag=self.tag)
            ...
            Input(self.canvas, 450, 250, 700, self.choices[1], self.handle_name_enter, self.name, self.tag)
        elif self.active_choice == 'exit':
            Prompt(self.canvas, 450, 250, 700, self.choices[2], self.handle_exit_input, self.handle_exit_enter, self.exit_prompt_index, self.tag)
